        ...
        <div class="content container-fluid p-4">
            ...
            <form @submit.prevent="submit">
                    ...
                    <div class="card-body py-3">
                        <div class="d-flex justify-content-between align-items-center">
                            <div class="d-flex align-items-center gap-3">
                                ...
                                <div>
                                    <h4 class="mb-0 fw-bold text-white">Tulis Berita Baru</h4>
                                    <p class="text-white-50 mb-0 small">Buat artikel menarik untuk pengunjung sekolah</p>
                                </div>
                            </div>
                            <div class="d-flex gap-2">
                                ...
                                <button type="submit" class="btn btn-warning px-4 py-2 fw-bold shadow-sm text-dark" :disabled="form.processing">
                                    ...
                                    {{ form.status === 'published' ? 'Terbitkan' : 'Simpan Draft' }}
                                </button>
                            </div>
                        </div>
                    </div>
                ...
                <div class="row g-4">
                            ...
                            <div class="card-body p-4">
                                <div class="form-group mb-4">
                                    ...
                                    <input 
                                        ...
                                        placeholder="Ketik judul berita yang menarik disini..." 
                                        v-model="form.title" 
                    ...
                    <div class="col-lg-4">
                        <!-- Publish Options Used to be here, now moved to sticky header or keep extra options here -->
                        <div class="card shadow-sm border-0 rounded-4 mb-4">
                            ...
                            <div class="card-body">
                                <div class="image-upload-area rounded-3" @click="$refs.fileInput.click()">
                                    <div v-if="!imagePreview" class="text-center p-5">
                                        ...
                                        <p class="mb-0 text-muted small">Klik area ini untuk memilih file</p>
                                    </div>
                                ...
                                <div class="alert alert-light border mt-3 mb-0 p-2 small text-muted d-flex align-items-start gap-2">
                                    ...
                                    <div>
                                        Rekomendasi rasio gambar <strong>16:9</strong> (Landscape).<br>Format JPG/PNG, Max 2MB.
                                    </div>
                                </div>
                            </div>
                        </div>
...
const submit = () => {
    form.post('/admin/posts', {
        onSuccess: () => {
            showToast('success', 'Post berhasil dibuat!');
        },
        onError: () => {
             showToast('error', 'Gagal membuat post. Periksa inputan anda.');
        }
    });
};
...
onMounted(() => {
    // We need to wait for Quill to be ready if we want to hook the toolbar manually, 
    // but vue-quill provides a 'modules' prop or we can access the instance via ref.
    // Simpler approach: define toolbar options with handler locally if supported, 
    // or correct way with vue-quill ref:
    if (quill.value) {
        const editor = quill.value.getQuill();
